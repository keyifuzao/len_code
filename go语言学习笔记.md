...
> [!IMPORTANT]
>
> 一次输入多个值的时候`printIn`中间有空格，`print`没有
>
> `printIn`会换行，`print`不会
>
> `printf`是格式化输出，在很多场景下比`println`更方便，`%V`代表数值，`%D`代表十进制，`%T`代表类型
...
- 数值类型
  ...
  字节也叫Byte，是计算机数据的基本存储单位。8bit（位）=1Byte（字节） 1024Byte = 1 KB  1024KB = 1MB  1024MB = 1GB  1024GB = 1TB，在电脑中，一个中文占用两个字节
